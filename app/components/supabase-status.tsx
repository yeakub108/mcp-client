...
export default function SupabaseStatus() {
  ...
  useEffect(() => {
    const checkConnection = async () => {
      try {
        ...
        const { data, error } = await supabase.from('test_connection').select('count').limit(1).maybeSingle();
        
        if (error && (error.message.includes('relation "test_connection" does not exist') || error.code === '42P01')) {
  ...
  return (
    <div className="bg-red-50 p-2 rounded-md text-sm">
      <div className="flex items-center">
        <svg className="h-4 w-4 mr-2 text-red-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
